...
import pandas as pd
from gemini import analyze_product
...
def analyze_youtube(url, progress_callback=None):
    ...
    def predict_progress(done, total):
        ...
        update(percent, f"🤖 Đang dự đoán... ({done}/{total})")

    predictions = predict_batch(comments, progress_callback=predict_progress)
    ...
    df = pd.DataFrame({
        "comment": comments,
        "predicted_labels": [", ".join(labels) if labels else "irrelevant" for labels in predictions]
    })
